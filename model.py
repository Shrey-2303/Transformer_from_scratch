...
class PositionalEncding(nn.Module):
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        pe = pe.unsqueeze(0) # add extra dim in the front for batches
        
        self.register_buffer('pe',pe) # register a tensor which is not a learnt parameter, like a variable being stored)
...
class FeedForwardBlock(nn.Module):
    def __init__(self, d_model: int, d_fft: int, dropout: float) -> None:
        super().__init__()
        self.linear_1 = nn.Linear(d_model, d_fft)
        self.dropout = nn.Dropout(dropout)
        self.linear_2 = nn.Linear(d_fft, d_model)
...
class MultiHeadAttentionBlock(nn.Model):
    ...
    def attention(query, key, value, mask, dropout:nn.Dropout):
        ...
        # (B,h, seq_len, seq_len) @ (B,h,seq_len, d_k) -> (b, h, seq_len, d_k)
        return (attention_scores @ value), attention_scores 
            
        
    def forward(self, q, k, v, mask):
        query = self.w_q(q) # same size as original
        key = self.w_k(k)
        value = self.w_v(v) 
        ...
        # return to original shape -> (B, seq_len, h, d_k) then (B, seq_len, h*d_k)
...
class EncoderBlock(nn.Module):
    ...
    # avoid interaction of padding word with other words
...
def build_transformer(source_vocab_size: int, 
    ...
    src_embed = InputEmbeddings(d_model, source_vocab_size)
    tgt_embed = InputEmbeddings(d_model, target_vocab_size)
    
    # create positional encoding layers
    src_pos = PositionalEncding(d_model, src_Seq_len, dropout)
    tgt_pos = PositionalEncding(d_model, tgt_seq_len, dropout)
    ...
    # create whole encoder and decoder
